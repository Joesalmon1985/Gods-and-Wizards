...
static func run(test_assert: TestAssert) -> void:
	...
	_scan_integration_files(test_assert)
...
static func _scan_integration_files(test_assert: TestAssert) -> void:
	for path in ArchitectureScanner.list_gd_files(ArchitectureScanner.INTEGRATION_ROOT):
		var lines := ArchitectureScanner.read_code_lines(path)
		for line in lines:
			if ArchitectureScanner.is_comment_only_line(line):
				continue
			test_assert.check(
				not ArchitectureScanner.line_contains_token(line, "extends Control"),
				"integration should avoid UI node types in %s" % path
			)
			for forbidden in ["GameState.new", "ActionRules.apply", "SetupRules.place_city"]:
				test_assert.check(
					not ArchitectureScanner.line_contains_token(line, forbidden),
					"integration must not mutate gameplay state (%s) in %s" % [forbidden, path]
				)
