...
static func run(test_assert: TestAssert) -> void:
	...
	var interior_vertex := BoardNode.from_hex_corner(HexCoord.new(0, 0), 0)
	...
	var edge_vertex := BoardNode.from_hex_corner(edge_hex, 0)
	var edge_hexes := board.get_hexes_for_vertex(edge_vertex)
	test_assert.eq(edge_hexes.size(), 2, "edge vertex should touch 2 hexes")

	var corner_vertices: Array[BoardNode] = []
	var edge_vertices: Array[BoardNode] = []
	...
	for coord in board.get_all_coords_sorted():
		for corner in range(6):
			var vertex := BoardNode.from_hex_corner(coord, corner)
