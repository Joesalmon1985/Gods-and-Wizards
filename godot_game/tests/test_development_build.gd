...
static func run(test_assert: TestAssert) -> void:
	...
	var action := _development_action(state, city_vertex)
...
static func _development_action(state: GameState, vertex: BoardNode) -> GameAction:
	for action in state.action_space.all_actions_sorted():
		if action.kind == ActionKind.Kind.BUILD_DEVELOPMENT and action.vertex.equals(vertex):
			return action
	return null
