"""Behavioural cloning trainer for micro_combat_v2 CSV."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train micro BC policy from micro_combat_v2 CSV")
    parser.add_argument("--csv", required=True, help="Path to micro_combat_v2 CSV export")
    parser.add_argument("--output-weights", required=True, help="Path for Godot-compatible weights JSON")
    parser.add_argument("--run-id", default="micro_bc_run", help="Checkpoint run identifier")
...
def main(argv: list[str] | None = None) -> int:
    ...
    dataset = load_micro_v2_csv(args.csv, output_size=args.output_size)
    ...
    training_cmd = " ".join(["python", "-m", "train.train_micro_bc", f"--csv={args.csv}", f"--output-weights={args.output_weights}"])
    eval_cmd = f"python -m eval.evaluate_micro_policy --checkpoint {weights_path} --episodes 10 --seed 901 --output logs/eval_micro_run_k.csv"
    metadata = checkpoint_metadata.new_metadata(
        run_id=args.run_id,
        policy_kind="micro_bc",
        ...
        spell_catalog_version=str(dataset.metadata.get("spell_catalog_version", "")),
    ...
    print(
        f"micro BC complete: loss={metrics['final_loss']:.4f} "
